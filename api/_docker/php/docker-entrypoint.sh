...
if [ "$1" = 'php-fpm' ] || [ "$1" = 'bin/console' ]; then
  ...
  mkdir -p var/cache var/log var/uploads config/jwt
    ...
    ln -sf /usr/local/etc/php/mods-available/php.dev.ini /usr/local/etc/php/conf.d/php.dev.ini
	  ...
	  composer install --prefer-dist --no-progress --no-suggest --no-interaction || EXIT_CODE=$? && true
    echo ${EXIT_CODE}
    ...
    php bin/console doctrine:migrations:diff --no-interaction --filter-expression='~^(?!session)~' || EXIT_CODE=$? && true
    echo ${EXIT_CODE}
    php bin/console doctrine:migrations:migrate --no-interaction --allow-no-migration || EXIT_CODE=$? && true
    echo ${EXIT_CODE}
