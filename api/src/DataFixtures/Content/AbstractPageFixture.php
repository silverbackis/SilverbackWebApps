...
abstract class AbstractPageFixture extends AbstractFixture implements DependentFixtureInterface
{
    ...
    protected function getPage(string $routeName, string $routePath): PageFixtureModel
    {
        ...
        $page = ($routeContent = $route->getContent()) ?: new Page();
        if (!$routeContent) {
            $page->addRoute($route);
        }
        $page
    ...
    protected function addComponent(AbstractComponent $component, ?AbstractContent $page = null, string $dynamicPageClass = null): ComponentLocation
    {
        $location = (new ComponentLocation($page))->setDynamicPageClass($dynamicPageClass);
